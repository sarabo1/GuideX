...
export class ServiceCitiesService {
  private cities_url =
    'https://data.gov.il/api/3/action/datastore_search?resource_id=3f06e2f2-e2ad-41ac-9665-37d0625537f2&limit=5';
    ...
    return this.http.get<any>(this.cities_url).pipe(
      map((response) => {
        if (response.success && response.result.records) {
          this.settlements = response.result.records.map(
            (record: { _id: any; שם_ישוב: string }) => ({
              id: record._id,
